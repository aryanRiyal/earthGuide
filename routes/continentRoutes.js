...
router.get('/', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).json({
            error: 'Internal Server Error - While fetching all Continent Information!!'
        });
    }
});
...
router.post('/new', async function (req, res) {
    ...
    if (ContinentsEnum.includes(data.name)) {
        try {
            ...
            console.error(err);
            res.status(500).json({
                error: 'Internal Server Error - While saving New Continent Information!!'
            });
        }
    } else {
...
router.get('/:name', async (req, res) => {
    ...
    if (ContinentsEnum.includes(continentName)) {
        try {
            const continentDetails = await Continent.findOne({ name: continentName }).populate(
                'wonders'
            );
            if (continentDetails) {
